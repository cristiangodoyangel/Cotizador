...
const Home = ({ onCreateCotizacion }) => {
  ...
  const handleAddItem = () => {
    if (detalle.trim() !== "" && cantidad > 0 && precioUnitario >= 0) {
      // Added validation
            ...
            .map(
              (item) =>
                `${item.id} - ${item.cantidad} - ${
                  item.detalle
                } - ${item.precioUnitario.toLocaleString()} - ${item.precioTotal.toLocaleString()}`
            )
            .join("\n"),
    ...
    <div className="home-form">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <ul>
          {items.map((item) => (
            <li key={item.id}>
              {item.id} - {item.cantidad} - {item.detalle} - $
